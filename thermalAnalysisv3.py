...
#analysis is based on energy balance derived from transient closed sytem 1st law princiapals 
def thermal_analysis(passtype,hbatteryArray,havionicArray,altitude, latitude, day, solarTime, airSpeed, totalTime, vent_velocity,
    ...
    viewfactorMatrix = viewfactors.avionics_view_factors(20, filepath)
 ...
 # only initialize if arrays were not passed in
    if passtype == "first pass" or len(hbatteryArray) == 0:
        havionicArray = np.zeros(nsteps)
        hbatteryArray = np.zeros(nsteps)
       
    hbatteryArrayPlot = np.zeros(nstore)
    havionicArrayPlot = np.zeros(nstore)
    ...
    for w in range(nsteps):
        ...
        #first pass to call to find htcs 
        ...
        if (w % storeEverySteps) == 0 and passtype=="first pass":
            if z < len(hbatteryArrayPlot): 
                hbatteryArrayPlot[z] = hbattery
                havionicArrayPlot[z] = havionic
                timeArray[z] = tcurrent / 3600.0
                print(f"Time computed: {tcurrent:.2f} s")
                z += 1   
            
        #second pass to find temperature plots 
        if passtype=="second pass":
            
            #get the htcs
            if w >= len(havionicArray):
                break
            havionic=havionicArray[w]
            hbattery=hbatteryArray[w]
            ...
            for i in range(Ncomp):
                if i == Ncomp-1:
                            ...
                            Qnet[i]=qnet[i]*areas_comp[i]+QgenTotal[i]-qhsConduction #net radiation + heat generation - conduction 
                            avionicTemperatures[i] += delta * ((Qnet[i])/cap) #avionic/battery energy balance 
                            dTdt[i] = (Qnet[i]) / cap #rate of temperature change for each avionic or battery 
    ...
    return (
        ...
        silvusTemp, silvusNetPower, TwallinsideNetPower, hbatteryArray, havionicArray,hbatteryArrayPlot,havionicArrayPlot
    )
